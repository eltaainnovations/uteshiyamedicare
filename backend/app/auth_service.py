...
def _deliver_otp(email: str, code: str) -> None:
    # TODO(follow-up): send via real SMTP once email infra exists (see
    # PROJECT_CONTEXT.md admin "Email Management"). Logged for now so the
    # flow is testable end-to-end without SMTP creds.
    logger.info("2FA code for %s: %s", email, code)
...
async def authenticate_credentials(email: str, password: str) -> tuple[PortalUser, str]:
    ...
    user = PortalUser(email=email.lower(), name=erpnext_session.full_name, role=role)
...
def issue_session(user: PortalUser, encrypted_sid: bytes) -> tuple[str, int]:
    ...
    token, _hard_ceiling_seconds = create_access_token(subject=user.email, role=user.role, jti=jti)
    ...
    return token, settings.session_timeout_minutes * 60
...
def touch_session(jti: str) -> SessionRecord:
    ...
    now = datetime.now(timezone.utc)
    if now - record.last_activity > timedelta(minutes=settings.session_timeout_minutes):
